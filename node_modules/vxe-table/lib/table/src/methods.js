...
var Methods = {
  ...
  loadChildren: function loadChildren(row, childRecords) {
    ...
    return this.createData(childRecords).then(function (rows) {
      var keepSource = _this6.keepSource,
          tableSourceData = _this6.tableSourceData,
          treeOpts = _this6.treeOpts,
          fullDataRowIdData = _this6.fullDataRowIdData,
          fullDataRowMap = _this6.fullDataRowMap,
          fullAllDataRowMap = _this6.fullAllDataRowMap,
          fullAllDataRowIdData = _this6.fullAllDataRowIdData;
      ...
      if (keepSource) {
        ...
        if (matchObj) {
          matchObj.item[children] = _ctor.default.clone(rows, true);
        }
      }

      _ctor.default.eachTree(rows, function (childRow, index, items, path, parent) {
        var rowid = getRowid(_this6, childRow);
        var rest = {
          row: childRow,
          rowid: rowid,
          index: -1,
  ...
  createData: function createData(records) {
    ...
    var rows = treeConfig ? _ctor.default.mapTree(records, handleRrecord, treeOpts) : records.map(handleRrecord);
    return this.$nextTick().then(function () {
      return rows;
    });
  },
      ...
      if (!isRemote) {
        if (allSortMethod) {
          ...
          var rest = column.sortMethod ? tableData.sort(column.sortMethod) : _ctor.default.sortBy(tableData, column.sortBy || (column.formatter ? function (row) {
            return _this11.getCellLabel(row, column);
          } : column.property));
          ...
          if (validTip && getEventTargetNode(evnt, validTip.$el).flag) {// 如果是激活状态，且点击了校验提示框
            ...
            if (!getEventTargetNode(evnt, document.body, 'vxe-table--ignore-clear').flag) {
              ...
              this.preventEvent(evnt, 'event.clearActived', actived.args, function () {
                ...
                if (!isClearActived && _this19.height && !_this19.overflowY) {
  ...
  handleGlobalKeydownEvent: function handleGlobalKeydownEvent(evnt) {
    ...
    if (this.isActivated) {
      this.preventEvent(evnt, 'event.keydown', null, function () {
        var filterStore = _this20.filterStore,
            isCtxMenu = _this20.isCtxMenu,
            ctxMenuStore = _this20.ctxMenuStore,
            editStore = _this20.editStore,
            editOpts = _this20.editOpts,
            editConfig = _this20.editConfig,
            mouseConfig = _this20.mouseConfig,
            mouseOpts = _this20.mouseOpts,
            keyboardConfig = _this20.keyboardConfig,
            keyboardOpts = _this20.keyboardOpts,
        ...
        if (operCtxMenu) {
        ...
        } else if (keyboardConfig && mouseConfig && mouseOpts.area && _this20.handleKeyboardEvent) {
            ...
            if (selected.row || actived.row) {
              ...
              if (hasShiftKey) {
                if (keyboardOpts.enterToTab) {
                  _this20.moveTabSelected(targetArgs, hasShiftKey, evnt);
                } else {
                  _this20.moveSelected(targetArgs, isLeftArrow, true, isRightArrow, false, evnt);
                }
              } else {
                if (keyboardOpts.enterToTab) {
                  _this20.moveTabSelected(targetArgs, hasShiftKey, evnt);
                } else {
                  _this20.moveSelected(targetArgs, isLeftArrow, false, isRightArrow, true, evnt);
                }
              }
          ...
          // 如果按下了 Tab 键切换
          if (selected.row || selected.column) {
            _this20.moveTabSelected(selected.args, hasShiftKey, evnt);
          } else if (actived.row || actived.column) {
            _this20.moveTabSelected(actived.args, hasShiftKey, evnt);
          }
          ...
          if (!isEditStatus) {
            ...
            if (keyboardOpts.isDel && (selected.row || selected.column)) {
              if (delMethod) {
                delMethod({
                  row: selected.row,
                  rowIndex: _this20.getRowIndex(selected.row),
                  column: selected.column,
                  columnIndex: _this20.getColumnIndex(selected.column),
                  $table: _this20
                });
              } else {
          ...
          if (selected.column && selected.row && _tools.UtilTools.isEnableConf(selected.column.editRender)) {
  ...
  isFilter: function isFilter(fieldOrColumn) {
    var column = _ctor.default.isString(fieldOrColumn) ? this.getColumnByField(fieldOrColumn) : fieldOrColumn;
  ...
  handleAsyncTreeExpandChilds: function handleAsyncTreeExpandChilds(row) {
    ...
    return new Promise(function (resolve) {
      ...
      loadMethod({
        ...
        if (!_ctor.default.isArray(childRecords)) {
          childRecords = [];
        }

        if (childRecords) {
          _this34.loadChildren(row, childRecords).then(function (childRows) {
            ...
            if (!checkStrictly && _this34.isCheckedByCheckboxRow(row)) {
              _this34.setCheckboxRow(childRows, true);
            }
          });
        }
